...
// Este código sólo es usado para mostrar la tabla de forma bonita
...
class VariadicTable {
  ...
  void print(StreamType& stream) {
    ...
    for (unsigned int i = 0; i < _num_columns; i++) {
      ...
      stream << std::string(_cell_padding, ' ') << std::setw(_column_sizes[i]) << std::left << std::string(half, ' ') + _headers[i]
